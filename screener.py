...
def getDataM(listado, start='2000-01-02', interval='1d', end=None):
    data = yf.download(listado, start=start, end=end, interval=interval, auto_adjust
                       =True)
    return data.swaplevel(i=1, j=0, axis=1)

#Seteo el USER : PASS @ HOST / BBDD_NAME
sql_engine = creat_engine('mysql+pymysql://root:@/screener_v1')
...
def getDataByYfinance(sp500_tickers):
    ...
    for ticker in sp500_tickers:
        ...
        tabla['vol_mln'] = tabla.Volume * tabla.Close / 1_000_000
...
tabla_full.to_sql(con=sql_conn, name='sp500', if_exists='replace', chunksize=100)
